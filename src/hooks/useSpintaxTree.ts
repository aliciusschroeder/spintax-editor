...
interface UseSpintaxTreeReturn {
  ...
  updateNode: (
    path: SpintaxPath, 
    newNodeOrFn: SpintaxNode | ((currentNode: SpintaxNode | null) => SpintaxNode | null)
  ) => void;
 ...
 */
export const useSpintaxTree = (initialSpintax?: string): UseSpintaxTreeReturn => {
  ...
  const getNodeByPath = useCallback((nodePath: SpintaxPath): SpintaxNode | null => {
    ...
    try {
      ...
      console.error("Error in getNodeByPath:", e);
      return null;
    }
  }, [tree]);
  ...
  const updateNode = useCallback((
    ...
    setTree(prevTree => {
      try {
        ...
        if (path.length === 0) {
          const updatedNode = typeof newNodeOrFn === 'function'
            ? newNodeOrFn(newTree)
            : newNodeOrFn;
          ...
          const rootNode = updatedNode as RootNode;
          updateStateFromTree(rootNode);
          return rootNode;
        }
        ...
        const newNode = typeof newNodeOrFn === 'function'
  ...
  }, [getNodeByPath, updateStateFromTree]);
  ...
  const addTextToRoot = useCallback((content: string = 'new text') => {
    ...
    setTree(prevTree => {
      try {
        ...
        newTree.children.push(newTextNode);
        updateStateFromTree(newTree);
        return newTree;
      } catch (e) {
        const errorMsg = e instanceof Error ? e.message : "Unknown error";
  ...
  const addChoiceToRoot = useCallback((options: string[] = ['A', 'B']) => {
    ...
    setTree(prevTree => {
      try {
        ...
        newTree.children.push(newChoiceNode);
        updateStateFromTree(newTree);
        return newTree;
      } catch (e) {
        const errorMsg = e instanceof Error ? e.message : "Unknown error";
